...
jsonfile = 'json.txt';
topicWordFile = 'topicwords.txt';

[treeResult] = MakeResults(hdpTree, vob); 
mkhvhdptrees(treeResult,jsonfile,topicWordFile);

numtopicf = 'numtopic.txt';
numtopic = cellfun(@(hdp) hdp.numclass, hdpTree);
fid = fopen(numtopicf,'w');
fprintf(fid,'number of topics: %s',mat2str(numtopic));
fclose(fid);